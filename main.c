        ...
        if (filterName == NULL || _tcsstr(pe32.szExeFile, filterName) != NULL) {
            ...
            if (hProcess != NULL) {
                if (GetProcessHandleCount(hProcess, &handleCount) && GetProcessTimes(hProcess, &ftCreation, &ftExit, &ftKernel, &ftUser)) {
                    ...
                    printf("| %-32s | %6lu | %4ld | %4lu | %7lu | %2c%02u:%02u:%02u.%03u | %4c%02u:%02u:%02u |\n",
                           pe32.szExeFile,
                           pe32.th32ProcessID,
                           pe32.pcPriClassBase,
                           pe32.cntThreads,
...
// ici on print les details de tous threads si -d est utilisé sans argument
BOOL print_all_thread_details(BOOL detailFlag, const char* filterName)
{
    printf("\n");
    // Elle créée un clicjés instantané sur les processus en cours d'execution
    // Le second argument, 0, spécifie que l'instantané concerne tous les processus du système. La fonction retourne un HANDLE vers l'instantané créé.
    ...
    //tant que des processus existent on affiche les informations detaillees sur les threads actuel
    do {
        ...
        do {
            if (te32.th32OwnerProcessID == pe32.th32ProcessID) { // Check if the thread belongs to the current process
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0) {
            ...
            printf("\nError in arguments: %s\nDo -h or --help for more infromations\n\n",argv[i]);
            helpFlag = TRUE;
        }
    }
    ...
    if(detailFlag) {
        if (pid == -1)
        {
            if (!print_all_thread_details(detailFlag, filterName)) {
